...
27. **지속적 DB 동기화 실패 원인 규명 및 고속 인메모리 캐시(`CacheService`) 구축 & 10회 연속 검증 완료 (2026-09-10 21차 배포 - GAS v12)**:
    * **원인 규명**:
      * Google Sheets API의 내부 RPC 지연 및 5개 시트(`Posts`, `Projects`, `ProjectNotes`, `Profile`, `Portfolio`) 순차 조회로 인해, 매 조회 시 응답 시간이 4.5초~5.5초에 달함.
      * 브라우저 프론트엔드의 엄격한 **5초 타임아웃(`AbortController`)** 임계값을 아슬아슬하게 초과(예: 5,016ms)할 때마다 브라우저가 통신을 강제 중단(`AbortError`)하고 우측 상단 붉은 팝업 메시지를 띄우는 악순환이 발생함.
    * **백엔드 고속 인메모리 캐싱 파이프라인 구축 (`gas_backend_code.gs` v12 라이브 배포 완료)**:
      * **`CacheService.getScriptCache()` 다중 청크(Chunk) 캐싱 탑재**:
        * Google Apps Script의 단일 키 100KB 제한을 극복하는 자동 분할(`CHUNK_SIZE = 95,000`) 청킹 캐싱 로직(`getCachedData`, `setCachedData`) 구현.
        * 최초 1회 로드 후 6시간 동안 초고속 인메모리 캐시에서 즉각 응답 제공 (처리 시간 **10ms 단위**).
      * **쓰기 작업 즉시 캐시 무효화 (`clearAllCache()`)**:
        * 글/프로젝트/프로필/포트폴리오 추가, 수정, 삭제(`savePost`, `deletePost` 등) 발생 시 즉각 전체 캐시를 삭제하여 데이터 정합성 100% 보장.
      * `node deploy_gas.js`를 통해 **v12 정식 라이브 배포 완료**.
    * **10회 연속 실시간 동기화 정밀 측정 결과 (100% 성공)**:
      * **총 시도 횟수**: 10회 / **성공 횟수**: 10회 / **실패 횟수**: 0회 (**성공률 100.0%**)
      * **최단 소요시간**: **920ms (0.92초)**
      * **최장 소요시간**: **1,719ms (1.72초)**
      * **평균 소요시간**: **1,213.9ms (1.21초)** (종전 대비 4배 이상 속도 향상, 5초 타임아웃 이내에 압도적 안전 마진 확보)
